...
class CSVReaderTest {
    ...
    void whenFilterTwoColumns(@TempDir Path folder) throws Exception {
        ...
        String string = Files.readString(target.toPath());
    ...
    @Test
    void whenGetNames(@TempDir Path folder) throws IOException {
        String data = String.join(
                System.lineSeparator(),
                "name,age,last_name,education",
                "Tom,20,Smith,Bachelor",
        ...
        List<String> result = CSVReader.getNames(argsName);
        List<String> expected = List.of("name", "age", "last_name", "education");
        assertThat(result).isEqualTo(expected);
    }
    @Test
    void whenGetColumnsNumbers() {
        Map<String, Integer> map = Map.of("name", 0, "education", 3);
        List<String> list = List.of("name", "education");
        List<Integer> result = CSVReader.getColumnsNumbers(map, list);
