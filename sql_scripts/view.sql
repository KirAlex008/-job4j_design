...
create view students_that_have_certain_book as
select  s.name as student, count(b.name) as amount from students as s
        join orders o on s.id = o.student_id
        join books b on o.book_id = b.id
        join authors a on b.author_id = a.id
    where b.name = 'Дубровский' or b.name = 'Вий' or b.name = 'Капитанская дочка'
    group by (s.name);

select * from students_that_have_certain_book;

alter view students_that_have_certain_book rename to student_book;
drop view student_book;
